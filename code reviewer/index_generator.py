...
class ReviewIndex:
    ...
    # Classic FreeBSD directories (used as hints when present)
    FREEBSD_TOP_DIRS = [
    ...
    @property
    def TOP_DIRS(self):
        """Auto-detect top-level directories containing reviewable code."""
        # Auto-discover all top-level subdirectories
        discovered = []
        skip = {'.git', '.ai-code-reviewer', '.angry-ai', 'ai-code-reviewer',
                'angry-ai', '__pycache__', '.venv', 'node_modules', '.reviewer-log'}
        try:
            for item in sorted(self.source_root.iterdir()):
                if item.is_dir() and item.name not in skip and not item.name.startswith('.'):
                    discovered.append(item.name)
        except PermissionError:
            pass

        # If we also have source files in the root, add "." as a pseudo-directory
        if not discovered:
            discovered = ['.']
    ...
    def generate(self) -> None:
        ...
        for top_dir in self.TOP_DIRS:
            if top_dir == '.':
                # Scan root directory itself for source files
                c_files, h_files, reviewable_found = self._scan_directory_contents(
                    self.source_root, '.')
                if reviewable_found:
                    total_lines = self._count_lines(c_files + h_files)
                    entry = DirectoryEntry(
                        path='.', c_files=len(c_files), h_files=len(h_files),
                        total_lines=total_lines)
                    self._restore_existing_status(entry, '.', existing_status)
                    self.entries['.'] = entry
                continue
            ...
            # Also check if the top_dir itself contains reviewable files directly
            c_files, h_files, reviewable_found = self._scan_directory_contents(
                top_path, top_dir)
            if reviewable_found:
                total_lines = self._count_lines(c_files + h_files)
                entry = DirectoryEntry(
                    path=top_dir, c_files=len(c_files), h_files=len(h_files),
                    total_lines=total_lines)
                self._restore_existing_status(entry, top_dir, existing_status)
                self.entries[top_dir] = entry
